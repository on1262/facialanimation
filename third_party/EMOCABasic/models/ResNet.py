...
def load_ResNet50Model():
    ...
    copy_parameter_from_resnet(model, resnet50(weights=ResNet50_Weights.IMAGENET1K_V1).state_dict())
    return model

def load_ResNet101Model():
    model = ResNet(Bottleneck, [3, 4, 23, 3])
    copy_parameter_from_resnet(model, resnet101(weights=ResNet101_Weights.DEFAULT).state_dict())
    return model

def load_ResNet152Model():
    model = ResNet(Bottleneck, [3, 8, 36, 3])
    copy_parameter_from_resnet(model, resnet152(weights=ResNet152_Weights.DEFAULT).state_dict())
    return model

...
# ablation study: 去掉transformer模块观察效果
...
class StyleExtractor(nn.Module):
    def __init__(self, dim_hidden, dim_output, num_heads, n_blocks, dp):
        ...
        self.mask_ebd = nn.Parameter(torch.zeros((1,1, dim_hidden)), requires_grad=False) # first token is ready to be filled with output
...
class Model(nn.Module):
    def __init__(
        ...
        self.param_predictor = ParamPredictor(dim_style=0, dim_aud=self.hidden, dim_out=56)
        ...
        self.opt_pred = optim.Adam(list(self.pred_layer.parameters(recurse=True)))
    ...
    def forward(self, in_dict):
        ...
        param_out = self.param_predictor(wav_out)
    ...
    def get_opt_list(self):
        return [self.opt_gen, self.opt_pred]

...
class Model(nn.Module):
    ...
    def batch_forward(self, in_dict): # used in training phase
        # regenerate emotion tensor for BIWI Dataset
        if 'domain' in in_dict.keys():
            for idx in range(in_dict['seqs_len'].size(0)):
                if in_dict['domain'][idx].lower() == 'biwi':
                    start_emo_pos = sum(in_dict['seqs_len'][:idx])
                    end_emo_pos = start_emo_pos + in_dict['seqs_len'][idx]
                    in_dict['emo_tensor'][start_emo_pos:end_emo_pos,:] = \
                        self.forward(in_dict['wav'][[idx],:], in_dict['seqs_len'][[idx]], None,  pred_only=True).detach().clone()
        
        return self.forward(in_dict['wav'], in_dict['seqs_len'], in_dict['emo_tensor'], in_dict['code_dict'])
    ...
    def test_forward(self, in_dict):
        ...
        if 'code_dict' in in_dict.keys() and in_dict['code_dict'] is not None:
            code_dict_list = [in_dict['code_dict']]
        else:
            code_dict_list = None
        if in_dict.get('emo_tensor') is None:
            in_dict['emo_tensor'] = self.forward(in_dict['wav'], in_dict['seqs_len'], None,  pred_only=True)
            #print(in_dict['emo_tensor'].size(), ' ', in_dict['seqs_len'][0].size())
            assert(in_dict['emo_tensor'].size(0) == torch.sum(in_dict['seqs_len']))
        ...
        assert(in_dict['emo_tensor'].size(0) == torch.sum(in_dict['seqs_len']))
        smooth = False if 'smooth' not in in_dict.keys() or in_dict['smooth'] != True else True
        out =  self.forward(in_dict['wav'], in_dict['seqs_len'], in_dict['emo_tensor'], code_dict_list, smooth=smooth)
        return out
    
    def forward(self, wavs, seqs_len, emo_tensor=None, code_dict=None, pred_only=False, smooth=False):
        ...
        exp_out = self.lstm_layer(wav_out + emo_ebd)
        zero_fill = torch.zeros((exp_out.size(0), exp_out.size(1), self.params_channels-self.cp_pred-6)).to(dev)
        pose_out = self.pose_layer(wav_out.detach())
        lstm_out = torch.cat([exp_out, zero_fill, pose_out], dim=-1)

        # smooth, only enable in test phase
        if smooth:
            lstm_out = torch.nn.functional.conv1d(
                lstm_out.permute(0,2,1), torch.ones((56,1,3), device=lstm_out.device)/3, padding=1, groups=56).permute(0,2,1)

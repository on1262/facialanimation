...
class Model(nn.Module):
    def __init__(
        ...
        self.style_layer = StyleTokenLayer(params_channels, out_hidden=768, in_hidden=768, ebd_size=10, dp=dp) # n_head*(ebd_size-1) should be constant
    ...
    def test_forward(self, wavs, seqs, seqs_len):
        # TODO seqs_gt should be replaced as attention softmax
        return self.batch_forward(wavs, seqs, seqs_len)

    def forward(self, wavs, seqs_gt, seqs_len):
        # input check
        assert(wavs.dim() == 2)
        ...
        tf_style_ebd = self.style_layer(seqs_gt, seqs_len)
        
        tf_out = tf_input + self.tf_layer(tf_input, cond_wav_fea, tf_style_ebd) # residual connection

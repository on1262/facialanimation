...
class StyleTokenLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_hidden=768, in_hidden=768, ebd_size=10, emo_class=6, dp=0.1):
        super().__init__()
        self.ln = LayerNorm(56)
        self.lstm = nn.LSTM((in_channels), (in_hidden), batch_first=True, dropout=dp)
        self.style_embedding = nn.Parameter(torch.zeros((1,ebd_size, in_hidden)), requires_grad=True)
        torch.nn.init.xavier_normal_(self.style_embedding)
        self.attn_layer = AdditiveAttention(in_hidden)
        
    
    def forward(self, x, seqs_len, style_attn=None):
        if style_attn is None:
            # input: batch_size, params_len, 56
            x = self.ln(x)
            x, _ = self.lstm(x)
            out = torch.zeros((x.size(0), 1, x.size(2)), device=x.device)
            for idx, length in enumerate(seqs_len):
                out[idx, :, :] += x[idx, length-1, :]
            #print('style x (batch_size,1,256) = ', x.size())
            ebd = self.style_embedding.expand(out.size(0), self.style_embedding.size(1), self.style_embedding.size(2)).to(x.device)
            #print('ebd size(batch_size, 10, 256)=', ebd.size())
            context, attn = self.attn_layer(out, ebd, ebd)
            # context: batch_size, 1, out_hidden
            # attn: batch_size, 
            return context, attn 
        elif x is None: 
            try:
                assert(style_attn.size() == (seqs_len.size(0), self.style_embedding.size(1))) # batch, ebd_size
            except Exception as e:
                print('style attn', style_attn.size(), ' seqs_len', seqs_len.size())
                print(self.style_embedding.size())
            ebd = self.style_embedding.expand(seqs_len.size(0), self.style_embedding.size(1), self.style_embedding.size(2))
            style_attn = style_attn.to(ebd.device)
            context = torch.bmm(style_attn.unsqueeze(1), ebd)
            return context
        else:
            print('StyleTokenLayer: one of seq gt or style attn should be none')
...
class StyleConvertLayer(nn.Module):
    ...
    def forward(self, emo):
        ...
        return self.sf(out) # B,ebd_size, softmax
...
class Model(nn.Module):
    def __init__(
        ...
        self.style_layer = StyleTokenLayer(params_channels, out_hidden=256, in_hidden=256, ebd_size=self.style_ebd_size, dp=dp)
        self.style_convert_layer = StyleConvertLayer(self.emo_class, self.style_ebd_size)
        # opts
    ...
    def batch_forward(self, in_dict):
        return self.forward(in_dict['wav'], in_dict['seqs'], in_dict['seqs_len'], in_dict['emo_tensor'])
        
    def test_forward(self, in_dict):
        assert(in_dict['wav'].size(0) == 1) # batch_size should be 1
        return self.forward(in_dict['wav'], None, in_dict['seqs_len'], in_dict['emo_tensor'])
    ...
    def forward(self, wavs, seqs_gt, seqs_len, emo_tensor=None):
        ...
        pred_attn = self.style_convert_layer(emo=emo_tensor)
        output['pred_attn'] = pred_attn

        if seqs_gt is not None: # use params gt predict attn
            seqs_gt = ((seqs_gt - self.out_norm['min']) / (self.out_norm['max'] - self.out_norm['min'])) - 0.5 # pre norm
            style_ebd, style_attn = self.style_layer(seqs_gt, seqs_len)
            assert(style_ebd.size(0) == wav_out.size(0) and style_ebd.size(1) == 1 and style_ebd.size(2) == wav_out.size(2))
            output['style_attn'] = style_attn
        else: # use emotion label predict attn
            style_ebd = self.style_layer(x=None, seqs_len=seqs_len, style_attn=pred_attn).to(wav_out.device)
        

        lstm_out = self.lstm_layer(wav_out + style_ebd)

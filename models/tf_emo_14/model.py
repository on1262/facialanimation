...
'''
tf_emo_14
在tf_emo_7上改进, 改进predictor, 消除表情预测的不稳定性.
...
class EmoPredLayer(nn.Module):
    ...
    def forward(self, x, style, seq_len):
        s = style.unsqueeze(1).repeat(1, x.size(1),1)
...
class Model(nn.Module):
    def __init__(
        ...
        self.pred_layer = EmoPredLayer(self.hidden+self.dim_style, self.emo_channels, dp=dp)
    ...
    def test_forward(self, in_dict):
        ...
        if in_dict['emo_tensor_conf'] == ['one_hot']:
            ...
            in_dict['emo_tensor_conf'] = ['overwrite']
    ...
    def forward(self, in_dict):
        ...
        pred = self.pred_layer(wav_out, style, seqs_len) # 梯度能够回传
        if pred_only:
            return pred
        else:
        ...
        if emo_tensor is not None:
            if emo_tensor_conf == ['use']:
                gt_e = emo_tensor
                gt_emo_tensor = torch.nn.functional.conv1d(
                    gt_e.unsqueeze(0).permute(0,2,1), torch.ones((7,1,31), device=gt_e.device)/31, padding=15, groups=7).permute(0,2,1).squeeze(0)
                emo_tensor = (pred.detach() + gt_emo_tensor) * 0.5 # predictor只受到out loss调控
        else:
            emo_tensor = pred
        
        if emo_tensor_conf == ['use'] and in_dict['emo_tensor'] is None:
        ...
        # 不存在no-use的情况, 所有都使用emotion tensor

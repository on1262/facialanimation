...
version 2:
dropout + deep attention + encoder transformer
delete self attention layer
...
class StyleTokenLayer(nn.Module):
    def __init__(self, in_channels, out_hidden=768, in_hidden=768, ebd_size=10, dp=0.1):
        super().__init__()
        self.ln = LayerNorm(56)
        self.lstm = nn.LSTM((in_channels), (in_hidden), batch_first=True, dropout=dp)
        self.style_embedding = nn.Parameter(torch.rand((1,ebd_size, in_hidden)), requires_grad=True)
    ...
    def forward(self, x, seqs_len):
        ...
        return context # batch_size, 1, out_hidden
...
class Model(nn.Module):
    def __init__(
        ...
        self.lstm_layer = LSTMLayer(64, params_channels,dp=dp)
        self.style_layer = StyleTokenLayer(params_channels, out_hidden=256, in_hidden=256, ebd_size=10, dp=dp)
        self.tf_layer = TransformerModel(
            n_params_fea=params_channels, n_wav_fea=self.wav_fea_channels, hidden=256, n_layers=4, attn_heads=1, dropout=dp)
        # opts
        ...
        self.opt_tf = optim.Adam(list(self.tf_layer.parameters(recurse=True)))
    ...
    def get_opt_list(self):
        return [self.opt_gen, self.opt_tf]
    ...
    def batch_forward(self, wavs, seqs, seqs_len):
        return self.forward(wavs, seqs, seqs_len)
        
    def test_forward(self, wavs, seqs_len):
        assert(wavs.size(0) == 1) # batch_size should be 1

        style = list(range(self.style_ebd_size))
        return self.forward(wavs, None, seqs_len, style)

    def forward(self, wavs, seqs_gt, seqs_len, style=None):
        # input check
        assert(wavs.dim() == 2)
        ...
        if style is None:
            ...
            style_ebd = self.style_layer(seqs_gt, seqs_len)
        else:
            style_ebd = self.style_layer(seqs_gt, seqs_len, style).to(wavs.device)
        
        tf_out = tf_input + self.tf_layer(tf_input, cond_wav_fea, style_ebd) # residual connection

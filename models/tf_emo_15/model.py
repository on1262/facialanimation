...
'''
tf_emo_15
根据tf_emo_7改进, 在不训练的情况下尽可能消除predictor预测的不准确性
predictor输出加一个大幅度平滑
'''
...
class EmoPredLayer(nn.Module):
    ...
    def smooth(self,x):
        return torch.nn.functional.conv1d(
            x.unsqueeze(0).permute(0,2,1), torch.ones((7,1,7), device=x.device)/7, padding=3, groups=7).permute(0,2,1).squeeze(0)
    ...
    def forward(self, x, seq_len):
        ...
        return self.smooth(out_emo_tensor) # not the last output
